...
> If the thrid-party dataset contains similar data with the GitHub maybe possible to included as "Derived" (Git-related sources).
...
- *Not related*: Mentioned dataset does not fit into "Direct" type or hard to traceback to find the detail of mentioned dataset (further than references level 2 from Snowballing technique).
...
- *Inherit*: Available only category "Both". The research paper applied the repository selection criteria to the third-party dataset whether the thrid-party dataset is related to GitHub or not then extend the dataset through GitHub.
  ...
  - The third-party is not related to GitHub: Left the dataset field empty.
...
> Should all dataset be extracted regardless the relationship to the Git-related VSC?
...
|:---:|---|:---:|:---:|
|  <b>1</b><sup>*<sup>  | Used dataset is collected from Git-related sources | <span style="background-color:darkgreen;padding:3px 32px;color:lightgreen;border-radius:4px;">Direct</span> | <span style="background-color:lightgreen;padding:3px 20px;color:darkgreen;border-radius:4px;">Have</span> |
...
|  <b>4</b>  | Used dataset include both Git-related sources and Git-related third-party dataset (not based on the third-party dataset) | <span style="background-color:purple;padding:3px 36px;color:violet;border-radius:4px;">Both</span> | <span style="background-color:lightgreen;padding:3px 20px;color:darkgreen;border-radius:4px;">Have</span> |
|  <b>5</b>  | Used dataset is based on Git-related third-party dataset then extend with Git-related sources with additional criteria | <span style="background-color:purple;padding:3px 36px;color:violet;border-radius:4px;">Both</span> | <span style="background-color:skyblue;padding:3px 6px;color:#006699;border-radius:4px;">Internal Inherit</span> |
|  <b>6</b>  | Used dataset is derived from non GitHub-related dataset that extend the dataset with data from Git-related sources with additional criteria | <span style="background-color:purple;padding:3px 36px;color:violet;border-radius:4px;">Both</span> | <span style="background-color:skyblue;padding:3px 4px;color:#006699;border-radius:4px;">External Inherit</span> |
...
|  <b>8</b>  | Used dataset is not collected from the Git-related source or used a third-party dataset which is not related to Git-related sources | <span style="background-color:grey;padding:3px 15px;color:beige;border-radius:4px;">Not related</span> | - |
---

<details>
  <summary>Terminology</summary>

  - Git-related sources: The version control system (VCS) like GitHub and other GitHub-like platforms, e.g., GitLab, BitBucket, Gerrit, etc.
  - Git-related third-party dataset: The third-party dataset which derived from Git-related sources.
</details>
<br>
...
**Use GitHub Dataset (Data source)**: The sources of dataset which can be classified into 5 types Direct, Derived, Both, Not related, and Not mentioned. If it is classified as "Not related" or "Not mentioned", the rest of classification will be skipped. <br>

**Repository Criteria Level**: If the paper describe the criteria for repositoy level selection, will be classified as "Have". Will be classified as "internal inherit" if the paper leverage the criteria from the GitHub-related third-party dataset, but if the paper apply the criteria from the non-GitHub third-party dataset, it will be classified as "external inherit". Otherwise will be classified as "Not have". <br>
...
**External Dataset**: The non-GitHub-related dataset name.
